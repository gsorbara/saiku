...
public class InfinispanSegmentCache extends MemorySegmentCache implements
		SegmentCache {

	private static final Logger LOGGER = LoggerFactory
			.getLogger(InfinispanSegmentCache.class);
	...
	public SegmentBody get(SegmentHeader header) {
		...
		ValueWrapper vw = springEmbeddedCacheManager.getCache(
				DEFAULT_CACHE_NAME).get(header);

		if (vw == null || vw.get() == null) {
			LOGGER.debug("Evict and return null for not hit cache by segment header: "
					+ header);
			springEmbeddedCacheManager.getCache(DEFAULT_CACHE_NAME).evict(
					header);
			return null;
		}

		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Hit cache and return " + vw.get()
					+ " by segment header: " + header);
		}
	...
	public List<SegmentHeader> getSegmentHeaders() {
		...
		// Set<SegmentHeader> headerSet = (Set<SegmentHeader>)
		// springEmbeddedCacheManager.getCache(DEFAULT_CACHE_NAME).getNativeCache().keySet();
	...
	public boolean put(final SegmentHeader header, SegmentBody body) {
		...
		springEmbeddedCacheManager.getCache(DEFAULT_CACHE_NAME).put(header,
				body);
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Put into cache " + body + " by segment header: "
					+ header);
		}
	...
	public boolean remove(final SegmentHeader header) {
		...
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Evict from cache " + ObjectUtils.getDisplayString(sb)
					+ " by segment header: " + header);
		}
		if (result) {
			fireSegmentCacheEvent(new SegmentCache.SegmentCacheListener.SegmentCacheEvent() {
